let objectArr = [
  ...
  let arr1 = objectArr.filter((value) => {
    return value.region.includes("Asia");
  });
  let arr2 = objectArr.filter((value) => {
    return value.population > 200000;
  });
  console.log("Data size: " + objectArr.length);
  console.log("Countries with region as Asia: " + arr1.length);
  console.log("Countries with population less than 2lac: " + arr2.length);
  
  objectArr.forEach((element)=>
  console.log("Name: " + element.name.common + "\nCapital: " + element.capital + "\nFlag: " + element.flag + "\n")
  );
  
  let sum1 = objectArr.reduce((previousValue, currentValue) => {
    return previousValue + currentValue.population;
  }, 0);
  console.log(sum1);
  
  let arr3 = objectArr.filter((value) => {
    return value.currencies?.USD !== undefined;
  });
  arr3.forEach((value) => console.log(value.name.common));
